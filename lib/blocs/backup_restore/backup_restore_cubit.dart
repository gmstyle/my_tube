...
class BackupRestoreCubit extends Cubit<BackupRestoreState> {
  ...
  Future<void> exportData() async {
    emit(state.copyWith(status: BackupRestoreStatus.loadingExport));
    try {
  ...
  Future<void> importData() async {
    emit(state.copyWith(status: BackupRestoreStatus.loadingImport));
    try {
      ...
      if (success) {
        ...
        emit(state.copyWith(
            status: BackupRestoreStatus.initial)); // User cancelled picker
      }
    } catch (e) {
      emit(state.copyWith(
